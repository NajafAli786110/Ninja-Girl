        ...
        <div className="w-full flex gap-6 gallery-container">
          <div className="w-full flex flex-wrap items-center justify-center gap-6">
            {teamMembers.map((member, index) => (
              <div
                key={index}
                className="team__item min-w-[48%] md:min-w-[22%] item py-12 px-8 flex flex-col items-center justify-center gap-8 bg-[#3b1e54]/20 border-2 border-white/10 rounded-3xl overflow-hidden relative"
              >
                <img
                  src={member.img}
                  alt={member.name}
